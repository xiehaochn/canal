...
import com.alibaba.otter.canal.connector.core.producer.atlas.AtlasMessageUtils;
...
public class CanalKafkaProducer extends AbstractMQProducer implements CanalMQProducer {
  ...
  private void loadKafkaProperties(Properties properties) {
    ...
    doMoreCompatibleConvert(
        "canal.mq.kafka.kerberos.krb5.file", "kafka.kerberos.krb5.file", properties);
    doMoreCompatibleConvert(
  ...
  @Override
  public void send(
      AuthenticationInfo mysqlAuthInfo,
            ...
            records.add(
                new ProducerRecord<>(
                    ...
                    CanalMessageSerializerUtil.serializer(
                        messagePartition, mqProperties.isFilterTransactionEntry())));
          }
        }
      } else {
        final int partition =
