...
public abstract class AbstractMQProducer implements CanalMQProducer {
   ...
   * 初始化配置
   *
   * <p>canal.mq.flat.message = true <br>
  ...
  private void loadCanalMqProperties(Properties properties) {
    ...
    String parallelBuildThreadSize =
        properties.getProperty(CanalConstants.CANAL_MQ_BUILD_THREAD_SIZE);
    ...
    String parallelSendThreadSize =
        properties.getProperty(CanalConstants.CANAL_MQ_SEND_THREAD_SIZE);
    ...
    String atlasMode = properties.getProperty(CanalConstants.CANAL_ATLAS_MODE);
    if (!StringUtils.isEmpty(atlasMode)) {
      mqProperties.setAtlasMode(Boolean.parseBoolean(atlasMode));
    }
  }
